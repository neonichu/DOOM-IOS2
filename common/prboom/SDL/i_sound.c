...
// I'm hacking back in the MIDI music support. At this time, I only want
// music-related functions, so I'm adding this macro to remove sfx-related
// functions.
#define ID_DISABLE_SDL_SOUND 1

#ifndef ID_DISABLE_SDL_SOUND
...
// Hack! Fix this if real SDL sound is reintroduced.
...
static char iphoneMusicPath[PATH_MAX];

#endif

void I_InitMusic(void)
{
	// Since we can load different IWADs in the same run of the iOS game, we need to clear
	// the cached lumpnums for all the music, since they differ in different IWADs.
  ...
  if (!music_tmp) {
	...
	iphoneMusicPath[0] = '\0';
	strcat( iphoneMusicPath, SysIphoneGetTempDir() );
	strcat( iphoneMusicPath, "prboom-music-XXXXXX" );
	
	music_tmp = strdup(iphoneMusicPath);
  #else
...
int I_RegisterSong(const void *data, size_t len)
{
  ...
  if ( memcmp(data, "MUS", 3) == 0 )
  {
    ...
    int musResult = mmus2mid(data, mididata, 89, 0);
	if ( musResult != 0 ) {
		printf( "Failed to conver MUS to MIDI: %s\n", music_tmp );
	}
